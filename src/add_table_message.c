...
#define SKY_ADD_TABLE_MESSAGE_TABLE_KEY_COUNT 2

struct tagbstring SKY_ADD_TABLE_MESSAGE_NAME_STR = bsStatic("name");
struct tagbstring SKY_ADD_TABLE_MESSAGE_TABLET_COUNT_STR = bsStatic("tabletCount");
...
// Creates an 'add_table' message object.
//
// Returns a new message.
sky_add_table_message *sky_add_table_message_create()
{
    sky_add_table_message *message = NULL;
    message = calloc(1, sizeof(sky_add_table_message)); check_mem(message);
    return message;

error:
    sky_add_table_message_free(message);
    return NULL;
}

// Frees a 'add_table' message object from memory.
//
...
sky_message_handler *sky_add_table_message_handler_create()
{
    ...
    handler->name = bfromcstr("add_table");
    handler->process = sky_add_table_message_process;
    return handler;
                                   ...
                                   sky_table *table, FILE *input, FILE *output)
{
    ...
    // Parse message.
    message = sky_add_table_message_create(); check_mem(message);
    message->data_path = bstrcpy(server->path); check_mem(message->data_path);
    message->table = sky_table_create();
    rc = sky_add_table_message_unpack(message, input);
    check(rc == 0, "Unable to parse 'add_table' message");
...
// Serializes an 'add_table' message to a file stream.
//
// message - The message.
...
int sky_add_table_message_pack(sky_add_table_message *message, FILE *file)
{
    ...
    // Map
    minipack_fwrite_map(file, SKY_ADD_TABLE_MESSAGE_TABLE_KEY_COUNT, &sz);
    check(sz > 0, "Unable to write map");
    
    // Name
    check(sky_minipack_fwrite_bstring(file, &SKY_ADD_TABLE_MESSAGE_NAME_STR) == 0, "Unable to write table name key");
    check(sky_minipack_fwrite_bstring(file, table_name) == 0, "Unable to write name value");

    // Tablet count
    check(sky_minipack_fwrite_bstring(file, &SKY_ADD_TABLE_MESSAGE_TABLET_COUNT_STR) == 0, "Unable to write tablet count key");
...
// Deserializes an 'add_table' message from a file stream.
//
// message - The message.
...
int sky_add_table_message_unpack(sky_add_table_message *message, FILE *file)
{
    ...
    for(i=0; i<map_length; i++) {
        ...
        if(biseq(key, &SKY_ADD_TABLE_MESSAGE_NAME_STR)) {
        ...
        else if(biseq(key, &SKY_ADD_TABLE_MESSAGE_TABLET_COUNT_STR)) {
