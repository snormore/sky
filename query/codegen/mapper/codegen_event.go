...
func (m *Mapper) codegenReadEventFunc() llvm.Value {
	...
	for i, decl := range read_decls {
		...
		m.store(m.builder.CreateGEP(m.load(ptr, ""), []llvm.Value{m.load(sz, "")}, ""), ptr)
		m.printf("event.read.value %d\n", m.constint(decl.Index()), m.load(field))
