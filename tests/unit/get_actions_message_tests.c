...
int test_sky_get_actions_message_unpack() {
    FILE *file = fopen("tests/fixtures/get_actions_message/0/message", "r");
    sky_get_actions_message *message = sky_get_actions_message_create();
    mu_assert_bool(sky_get_actions_message_unpack(message, file) == 0);
    fclose(file);
...
int all_tests() {
    mu_run_test(test_sky_get_actions_message_pack);
    mu_run_test(test_sky_get_actions_message_unpack);
    mu_run_test(test_sky_get_actions_message_process);
    return 0;
}
